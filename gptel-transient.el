...
Prompt for instructions and queue them with the state machine of
the most recent query, so the model receives them mid-generation.
Display the queued message in the query buffer via an overlay that
is removed after the next tool call."
  :key "M-RET"
